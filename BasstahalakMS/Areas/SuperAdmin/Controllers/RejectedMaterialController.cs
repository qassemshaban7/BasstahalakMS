...
namespace BasstahalakMS.Areas.SuperAdmin.Controllers
{
    ...
    public class RejectedMaterialController : Controller
    {
        private readonly ApplicationDbContext _context;

        public RejectedMaterialController(ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> Rejected(int id)
        {
            ...
            if (existingFile.status == -1)
            {
                var bfileNotess = await _context.BfileNotes
                                .Where(p => p.BfileId == existingFile.Id && p.status == 5|| p.status == 2 || p.status == 3 || p.status == 4)
                                .Include(x => x.BFile)
                                .Include(x => x.User)
                                .ToListAsync();
        ...
        public async Task<IActionResult> Rejected(BFile bFile)
        {
            try
            {
                string userId = User.FindFirst(ClaimTypes.NameIdentifier).Value;
